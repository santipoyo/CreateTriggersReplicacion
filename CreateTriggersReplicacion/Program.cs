        ...
        private static void EscribirLista()
        {
            ...
            try
            {
                ...
                //foreach (String item in storedProcedures)
                {
                    ...
                    comandos.Add("			where	id = object_id('dbo.trReplicaInsert" + item.Key + "') ");
                    comandos.Add("					and type = 'TR') ");
                    comandos.Add("begin ");
                    comandos.Add("	drop trigger dbo.trReplicaInsert" + item.Key + "");
                    comandos.Add("end ");
                    comandos.Add("go ");
                    comandos.Add(Environment.NewLine);

                    comandos.Add("create trigger dbo.trReplicaInsert" + item.Key);
                    comandos.Add("on MaxManager.dbo." + item.Key);
                    comandos.Add("after insert");
                    comandos.Add("as");
                    ...
                    File.WriteAllLines(strRuta2 + @"trReplicaInsert" + item.Key + ".sql", comandos.ToArray());
